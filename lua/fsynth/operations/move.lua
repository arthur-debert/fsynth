...
function MoveOperation:validate()
	...
	if self.source_is_symlink then
		logger.debug("Validate Move: Source '{}' is a symlink.", self.source)
		...
		if not pcall_ok or link_target_str == nil then
			...
			logger.debug("Validate Move: Source symlink '{}' points to '{}'.", self.source, self.source_symlink_target)
		end
		...
		if not self.was_directory then -- Source is a regular file
			logger.debug("Validate Move: Source '{}' is a file.", self.source)
			...
			logger.debug("Validate Move: Initial checksum for source file '{}': {}", self.source, result)
		else
			logger.debug("Validate Move: Source '{}' is a directory.", self.source)
...
function MoveOperation:execute()
	...
	if self.was_directory == nil or (pl_path.exists(self.target) and self.target_is_directory_move_into == nil) then -- Re-validate if critical flags aren't set
		logger.debug("Execute Move: Critical validation flags not set, re-validating.")
		local valid, err = self:validate()
		if not valid then
			logger.error("Execute Move: Validation failed for '%s' -> '%s': %s", self.source, self.target, err)
			return false, err
		end
	end
	...
	if self.target_is_directory_move_into then
		...
		logger.debug("Execute Move: Target is directory, actual_target_path set to '%s'", self.actual_target_path)
	end
	...
	if actual_target_existed_before_move then
		...
		logger.info("Execute Move: Target '%s' exists and will be overwritten.", self.actual_target_path)
	...
	if self.options.create_parent_dirs then
		...
		if parent_dir and parent_dir ~= "" and parent_dir ~= "." and not pl_path.isdir(parent_dir) then
			...
			logger.debug("Execute Move: Creating parent directory: %s", parent_dir)
			...
			logger.info("Execute Move: Parent directory created: %s", parent_dir)
		end
	end

	-- Perform the Move operation to actual_target_path
	logger.info("Execute Move: Moving '%s' -> '%s'", self.source, self.actual_target_path)
	...
	logger.info("Execute Move: Successfully moved '%s' to '%s'", self.source, self.actual_target_path)

	-- Checksum Target (if file-like and not a symlink)
	if not self.was_directory and not self.source_is_symlink then
		logger.debug("Execute Move: Calculating checksum for moved file: %s", self.actual_target_path)
	...
	elseif self.source_is_symlink then
		logger.debug("Execute Move: Source was a symlink, skipping final target checksumming for the link itself.")
...
function MoveOperation:undo()
	...
	logger.info("Undo Move: Moving '%s' -> '%s'", current_location, self.source)
	...
	logger.info("Undo Move: Successfully moved '%s' back to '%s'", current_location, self.source)
	...
	if not self.was_directory and not self.source_is_symlink and self.checksum_data.initial_source_checksum then
		logger.debug("Undo Move: Verifying checksum for restored source file: %s", self.source)
		...
		elseif restored_cs ~= self.checksum_data.initial_source_checksum then
			...
			logger.info("Undo Move: Checksum verified for restored source file '%s'", self.source)
		end
	elseif self.source_is_symlink then
		logger.debug("Undo Move: Source was a symlink. Verifying link target if available.")
