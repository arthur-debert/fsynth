...
function DeleteOperation:validate()
  ...
  if not pl_path.exists(self.source) then
    return false, "Path to delete '" .. self.source .. "' does not exist."
  end
  ...
  if self.original_path_was_directory then
    ...
    if not pcall_ok_files then return false, "Error checking directory files for '"..self.source.."': " .. tostring(pcall_val_files) end

    local pcall_ok_dirs, pcall_val_dirs = pcall(function() dirs = pl_dir.getsubdirs(self.source) end)
    if not pcall_ok_dirs then return false, "Error checking directory subdirs for '"..self.source.."': " .. tostring(pcall_val_dirs) end

    local is_not_empty = (files and next(files)) or (dirs and next(dirs))

    if self.options.is_recursive then
      if is_not_empty then
        return false, "Directory '" .. self.source .. "' is not empty and recursive delete with safety limits is not yet implemented. Only empty directory deletion is supported currently."
      end
    else 
      if is_not_empty then
        return false, "Directory '" .. self.source .. "' is not empty. Use is_recursive=true for empty directories or wait for full recursive delete functionality."
      end
    end
  else -- It's a file
    local content
    ...
    if content == nil and pcall_read_ok then 
      ...
      return false, "Failed to read file '" .. self.source .. "' for deletion: Penlight pl_file.read returned nil (possibly unreadable)."
    end
    ...
    if not pcall_cs_ok then
      return false, "Failed to calculate checksum for file '" .. self.source .. "' (pcall error): " .. tostring(pcall_cs_val)
    end
    if not cs_result then
      return false, "Failed to calculate checksum for file '" .. self.source .. "': " .. (pcall_cs_val or "checksum calculation failed")
    end
...
function DeleteOperation:execute()
  if not pl_path.exists(self.source) then
    self.item_actually_deleted = false 
    return true, "Path '" .. self.source .. "' already deleted."
  end
  ...
  if self.original_path_was_directory then
    ...
    if not pcall_ok then
      return false, "Failed to delete directory '" .. self.source .. "' (pcall error): " .. tostring(penlight_success) -- penlight_success is error from pcall
    end
    if not penlight_success then
      return false, "Failed to delete directory '" .. self.source .. "': " .. (penlight_errmsg or "unknown Penlight error")
    end
  else -- It's a file
    local os_remove_pcall_ok, os_remove_ret1, os_remove_ret2 = pcall(os.remove, self.source)
    if not os_remove_pcall_ok then
      return false, "Failed to delete file '" .. self.source .. "' (pcall error): " .. tostring(os_remove_ret1) -- ret1 is error from pcall
    end
    if not os_remove_ret1 then -- os.remove failed (returned nil, errmsg)
      return false, "Failed to delete file '" .. self.source .. "': " .. (os_remove_ret2 or "unknown OS error")
    end
  end
...
function DeleteOperation:undo()
  ...
  if pl_path.exists(self.source) then
    return false, "Undo: Path '" .. self.source .. "' already exists, cannot undo delete."
  end
  ...
  if self.original_path_was_directory then
    ...
    if not pcall_ok then
      return false, "Undo: Failed to recreate directory '" .. self.source .. "' (pcall error): " .. tostring(penlight_success)
    end
    if not penlight_success then
      return false, "Undo: Failed to recreate directory '" .. self.source .. "': " .. (penlight_errmsg or "unknown Penlight error")
    end
  else -- It was a file
    if self.original_content == nil then
      ...
      return false, "Undo: No original content stored (or content was nil), cannot undo file deletion for '" .. self.source .. "'"
    end

    pcall_ok, penlight_success, penlight_errmsg = pcall(pl_file.write, self.source, self.original_content)
    if not pcall_ok then
      return false, "Undo: Failed to restore file '" .. self.source .. "' (pcall error): " .. tostring(penlight_success)
    end
    if not penlight_success then
      return false, "Undo: Failed to restore file '" .. self.source .. "': " .. (penlight_errmsg or "unknown Penlight error")
    end
    ...
    if not cs_pcall_ok then
      return false, "Undo: Error calculating checksum for restored file '"..self.source.."' (pcall error): " .. tostring(cs_pcall_val)
    end
    if not current_checksum then
       return false, "Undo: Failed to calculate checksum for restored file '"..self.source.."': " .. (cs_pcall_val or "checksum calculation failed")
    end
    
    if current_checksum ~= self.checksum_data.original_checksum then
      return false, "Undo: Checksum mismatch after restoring file '" .. self.source .. "'. Content may be corrupt. Expected: " .. (self.checksum_data.original_checksum or "nil") .. ", Got: " .. (current_checksum or "nil")
    end
  end
