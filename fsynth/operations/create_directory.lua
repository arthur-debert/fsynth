...
function CreateDirectoryOperation:execute()
	...
	if pl_path.exists(parent_dir_path) and pl_path.isdir(parent_dir_path) then
		...
		if not writable then
			err_msg = fmt(
				"Parent directory '%s' is not writable. Error: %s",
				parent_dir_path,
...
function CreateDirectoryOperation:undo()
	...
	if not ok_files then
		return false, fmt("Undo: Error listing files in '%s': %s", self.target, tostring(err_files_or_data))
	end
	...
	if not ok_dirs then
		return false, fmt("Undo: Error listing subdirectories in '%s': %s", self.target, tostring(err_dirs_or_data))
	end
