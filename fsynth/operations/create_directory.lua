...
function CreateDirectoryOperation.new(dir_path, options)
  ...
  self.options.create_parent_dirs = self.options.create_parent_dirs == nil and true 
                                 or self.options.create_parent_dirs
  ...
  if pl_path.exists(self.target) then
    ...
    if not self.options.create_parent_dirs then
      ...
      if parent_dir and parent_dir ~= "" and parent_dir ~= "." and not pl_path.isdir(parent_dir) then
        local err_msg = fmt("Parent directory of '{}' does not exist and create_parent_dirs is false.", self.target)
        log.error(err_msg)
        return false, err_msg
      end
    end
...
function CreateDirectoryOperation:undo()
  ...
  if not ok_files then
    return false, fmt("Undo: Error listing files in '{}': {}", 
                      self.target, tostring(err_files_or_data))
  end
  ...
  if not ok_dirs then
    return false, fmt("Undo: Error listing subdirectories in '{}': {}", 
                      self.target, tostring(err_dirs_or_data))
  end
  ...
  if not rmdir_pcall_ok then
    local err_msg = fmt("Undo: Failed to remove directory '{}' (pcall error): {}", 
                      self.target, tostring(rmdir_pcall_val1))
    log.error(err_msg)
    return false, err_msg
  end
  if not rmdir_pcall_val1 then -- Penlight rmdir returned nil, msg
    local err_msg = fmt("Undo: Failed to remove directory '{}': {}", 
                       self.target, rmdir_pcall_val2 or "unknown Penlight error")
    log.error(err_msg)
    return false, err_msg
  end
