...
local pl_path = require("pl.path") -- Changed 'path' to 'pl_path' for clarity
...
describe("SymlinkOperation", function()
  ...
  local function create_dir_for_test(path_str, make_parents)
    if make_parents then
        assert.is_true(fs.mkdir(path_str, make_parents), "Failed to create directory for test setup: " .. path_str)
    else
        assert.is_true(fs.mkdir(path_str), "Failed to create directory for test setup: " .. path_str)
    end
  ...
  describe(":execute", function()
    describe("Successful Symlink Creation", function()
      it("should create a symlink to an existing file", function()
        ...
        local op = SymlinkOperation:new({ source = link_target_path_str, target = link_path_str })
        local success, err = op:execute()

        assert.is_true(success, err)
        assert.is_true(pl_path.islink(link_path_str))
        assert.are.equal(link_target_path_str, pl_path.readlink(link_path_str))
        assert.is_true(op.link_actually_created)
        assert.are.equal("this is the target", fs.read(link_path_str))
      end)
      ...
      it("should create a symlink to an existing directory", function()
        ...
        local op = SymlinkOperation:new({ source = link_target_dir_str, target = link_path_str })
        local success, err = op:execute()

        assert.is_true(success, err)
        assert.is_true(pl_path.islink(link_path_str))
        assert.are.equal(link_target_dir_str, pl_path.readlink(link_path_str))
        assert.is_true(op.link_actually_created)
        ...
        assert.are.equal("child content", fs.read(pl_path.join(link_path_str, "child.txt")))
      end)
    ...
    describe("options.overwrite = true", function()
      it("should replace an existing file with a symlink", function()
        ...
        local op = SymlinkOperation:new({
          source = link_target_path_str,
          target = link_path_str,
          overwrite = true
        })
        local success, err = op:execute()
      ...
      it("should replace an existing symlink with a new symlink", function()
        ...
        assert.is_true(fs.symlink(original_link_target_str, link_path_str))
        assert.is_true(pl_path.islink(link_path_str))
        assert.are.equal(original_link_target_str, pl_path.readlink(link_path_str))

        local op = SymlinkOperation:new({
          source = new_link_target_str,
          target = link_path_str,
          overwrite = true
        })
        local success, err = op:execute()
        ...
        assert.is_true(success, err)
        assert.is_true(pl_path.islink(link_path_str))
        assert.are.equal(new_link_target_str, pl_path.readlink(link_path_str))
        assert.is_true(op.link_actually_created)
        assert.is_true(op.original_target_existed_and_was_overwritten)
      end)
    end)
    ...
    describe("options.overwrite = false (default)", function()
      ...
      it("should fail if link path (self.target) already exists as a symlink", function()
        ...
        assert.is_true(fs.symlink(existing_sym_target_str, link_path_str)) 
        ...
        assert.match("Target path already exists", err)
        assert.is_true(pl_path.islink(link_path_str)) 
        assert.are.equal(existing_sym_target_str, pl_path.readlink(link_path_str))
        assert.is_false(op.link_actually_created)
      end)
    end)
    ...
    describe("Validation Failures", function()
      it("should fail if link target path (self.source) is not specified", function()
        ...
        local op = SymlinkOperation:new({ target = link_path_str })
      ...
      it("should fail if link path (self.target) is not specified", function()
        ...
        local op = SymlinkOperation:new({ source = link_target_path_str })
        local valid, err = op:validate()
        assert.is_false(valid)
        assert.match("Target path .* is required", err)
      end)

      it("should fail if link path (self.target) is an existing directory (cannot overwrite dir with symlink)", function()
        ...
        local op = SymlinkOperation:new({
          source = link_target_path_str,
          target = link_path_isdir_str,
          overwrite = true 
        })
        local valid, err_validate = op:validate()
        assert.is_false(valid_validate)
        assert.match("Target path .* is an existing directory and cannot be overwritten by a symlink", err_validate)

        local success_exec, err_exec = op:execute()
        assert.is_false(success_exec)
        assert.match("Target path .* is an existing directory and cannot be overwritten by a symlink", err_exec)
  ...
  describe(":undo", function()
    ...
    it("should be a no-op if link_actually_created is false", function()
      ...
      local op = SymlinkOperation:new({ source = link_target_path_str, target = link_path_str, overwrite = false })
      local exec_success, exec_err = op:execute()
    ...
    it("should fail (or be a no-op success) if link path (self.target) no longer exists at time of undo", function()
      ...
      assert.is_true(fs.delete(link_path_str))
    ...
    it("should fail if link path (self.target) is not a symlink at time of undo (e.g., replaced by a file)", function()
      ...
      assert.is_true(pl_path.exists(link_path_str)) 
      assert.are.equal("now a file", fs.read(link_path_str))
    end)

    it("should successfully restore original file if overwrite=true was used and original_target_was_file=true", function()
      ...
      assert.is_false(pl_path.islink(link_path_str))
      assert.is_true(pl_path.isfile(link_path_str))
      assert.are.equal(original_content, fs.read(link_path_str))
    end)

    it("should successfully restore original symlink if overwrite=true was used and original_target_was_symlink=true", function()
      ...
      assert.is_true(fs.symlink(original_symlink_points_to_str, link_path_str)) 
      ...
      assert.is_true(undo_success, undo_err)
      assert.is_true(pl_path.islink(link_path_str)) 
      assert.are.equal(original_symlink_points_to_str, pl_path.readlink(link_path_str)) 
    end)
  end)
end)
